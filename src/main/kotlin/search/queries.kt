...
class QueryContext(
    ...
    fun hits(): Hits = includeIds?.filter { true != excludeIds?.contains(it) }?.map { (it to 0) as Hit }
...
// class TermQuery(val field: String, val term: String) : Query {
...
class BoolQuery(
    val should: List<Query> = listOf(),
    val must: List<Query> = listOf(),
    val filter: List<Query> = listOf(),
    val mustNot: List<Query> = listOf()
) : Query {
    override fun hits(documentIndex: DocumentIndex, context: QueryContext): Hits {
        ...
        context.withFilterMode {
            ...
            val exclude = if(excludedHits.isNotEmpty())excludedHits.reduce(Hits::and).map { it.first } else emptyList()
        ...
        val mustHits = if (must.isEmpty() && filter.isNotEmpty()) {
            ...
            val mapped = must.map { it.hits(documentIndex, context) }
            if (mapped.isNotEmpty()) {
                mapped.reduce(Hits::and)
            } else {
                emptyList()
            }
        }
        ...
        val mapped = should.map { it.hits(documentIndex, context) }
        val shouldHits = if(mapped.isNotEmpty()) mapped.reduce(Hits::or) else emptyList()

        return when {
            must.isEmpty() && should.isEmpty() -> context.hits()
            filter.isEmpty() -> {
                when {
                    should.isEmpty() -> mustHits
                    must.isEmpty() -> shouldHits
                    else -> throw IllegalStateException("filter, should, and must were empty; check validation logic")
                }
            }
...
fun Hits.or(other: Hits): Hits {
    ...
    return collectedHits.entries.map { it as Hit }.filter { it.second > 0.0 }.sortedByDescending { it.second }
}
